...
def plot_msd_with_regression(csv_filename):
    ...
    plt.plot(data['Time'], data['Mean_MSD'], label='DCM', linestyle='-', color='b')

    filtered_data = data[(data['Time'] >= 0) & (data['Time'] <= 0.055)]
    ...
    slope_scientific_label = f"{slope / 10 ** slope_magnitude:.{decimals}f} x 10$^{{{int(slope_magnitude)}}}$"
    error_scientific_label = f"{std_err/10 ** (slope_magnitude):.{decimals}f}"

    line = (slope * filtered_data['Time']) + intercept


    plt.plot(filtered_data['Time'], line, label=f'm={slope_scientific_label} ± {error_scientific_label}', color='red', linestyle='--')
