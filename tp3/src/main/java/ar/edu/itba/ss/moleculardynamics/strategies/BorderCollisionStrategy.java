...
public class BorderCollisionStrategy implements CollisionStrategy<Border> {
    ...
    public Double handleCollision(MovableSurfaceEntity<Particle> particle, SurfaceEntity<Border> entity) {
        ...
        if(border.getP1().getX() == border.getP2().getX()){
            ...
            return (border.getP1().getX() - Math.signum(dirVect) * particle.getEntity().getRadius() - particle.getX())/particle.getXSpeed();
            ...
            return (border.getP1().getY() - Math.signum(dirVect) * particle.getEntity().getRadius() - particle.getY())/particle.getYSpeed();
