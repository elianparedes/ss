...
public class MarsVoyageSpaceship {
    ...
    public static void main(String[] args) throws IOException {
        ...
        try {
            builder.appendLine(fileName, "time", "name", "velocity", "x", "y", "distance_to_mars");
        } catch (IOException e) {
        ...
        while(Double.compare(time,start+stop) < 0) {
            ...
            if(Double.compare(time,start) >= 0  && (i% printI == 0)){
                Particle spaceship = state.get(3);
                Particle mars = state.get(2);
                double distance = mars.getPosition().sub(spaceship.getPosition()).norm();

                try {
                    builder.appendLine(
                            fileName,
                            String.valueOf(i),
                            String.valueOf(spaceship.getName()),
                            String.valueOf(spaceship.getVelocity().norm()),
                            String.valueOf(spaceship.getPosition().getX()),
                            String.valueOf(spaceship.getPosition().getY()),
