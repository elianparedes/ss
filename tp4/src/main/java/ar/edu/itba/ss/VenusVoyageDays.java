...
public class VenusVoyageDays {
     ...
     * Venus
     */
    private static final double VENUS_MASS = 4.867 * Math.pow(10, 24);
    private static final double VENUS_RADIUS = 6051.8;
    private static final double VENUS_X = 1.065472211052642E+08;
    private static final double VENUS_Y = 2.008820815762501E+07;
    private static final double VENUS_VX = -6.617590504936895E+00;
    private static final double VENUS_VY = 3.425594600887680E+01;
    ...
    public static void main(String[] args) throws IOException {
        ...
        // Initialize CSV File
        String fileName = String.format(argumentHandler.getArgument("-O") + "venus-min-distance.csv", dt, timeParameters.getRawStart() + timeParameters.getStartUnits());
        ...
        for (int s = 0; s < 100; s++) {
            ...
            particles.add(new Particle("venus", VENUS_MASS, VENUS_RADIUS, new Vector(VENUS_X, VENUS_Y), new Vector(VENUS_VX, VENUS_VY)));
            ...
            while (Double.compare(time, start + stop) < 0) {
                ...
                if (Double.compare(time, start) >= 0 && (i % printI == 0)) {
                    Particle venusState = state.get(3);
                    Particle spaceshipState = state.get(4);
                    minDistance = Math.min(minDistance, venusState.getPosition().sub(spaceshipState.getPosition()).norm());
                    minTimeIteration = i;
                }
    ...
    private static Particle initializeSpaceship(Particle earth) {
        ...
        Vector perpendicular = new Vector(position.getY(), -1 * position.getX());
