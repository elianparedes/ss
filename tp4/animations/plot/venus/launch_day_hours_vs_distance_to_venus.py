...
day_range = [f'{custom_date_fmt(value=float(day), _=None)}' for day in [197, 198, 199]]
split_day = custom_date_fmt(198, None)
current_day = custom_date_fmt(197, None)

csv_file = '../distances/venus-min-distance-hours.csv'
...
ax.set_ylabel('Mínima distancia entre la nave y Venus (km)')
ax.xaxis.set_major_formatter(ticker.ScalarFormatter(useMathText=True))
plt.yscale('log')

# Adjust x-axis ticks
ax.set_xticks(range(0, 48, 2))
ax.set_xticklabels([f'{hour % 24:02}:00' for hour in range(0, 48, 2)], rotation=45, ha='right')
...
ax.annotate(f'Mínima distancia: {min_distance:.2f} km\nel {current_day} a las {min_day}:00',
            xy=(min_day, min_distance), xytext=(min_day - 4, min_distance),
            arrowprops=dict(facecolor='black', arrowstyle='-'),
            fontsize=16, ha='right', bbox=bbox_props)
...
plt.savefig('../output/launch_day_hours_vs_distance_to_venus.png')
plt.show()
